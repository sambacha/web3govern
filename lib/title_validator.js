...
async function verifyTitles(context, config) {
  ...
  if (configKey in config) {
    ...
    const successMessage =
      verifyTitles.successMessage || "✅ Validation successful! ";
    // eslint-disable-next-line no-template-curly-in-string
    const failureMessage =
      verifyTitles.failureMessage || "Wrong ${type} title: ${title}";
    ...
    if (
      alwaysUsePrTitle ||
      (validateEitherPrOrSingleCommitTitle && messages.length !== 1)
    ) {
    ...
    for (const title of titles) {
      ...
      if (regexpMatch == null) {
        context.log.info(`${type} ⛔️ title is wrong:`, title);
        ...
        const message = failureMessage
          ...
          .join(title)
          .split("${regex}")
          .join(commitTitleRegexp.toString());
    ...
    context.log.info("✅ Commit/PR titles valid");
    await _createStatus(
      context,
      pr.head.sha,
      statusTitle,
