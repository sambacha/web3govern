...
async function addLabelsOnPr(context, config) {
  ...
  const defaultLabelerFlags = {
    labelOnPRUpdates: true,
    labelsOnPRCreation: [],
  };
  const labelerFlags =
    "labelerFlags" in config ? config.labelerFlags : defaultLabelerFlags;

  if (configKey in config) {
    const addLabelsOnPrConfig = config[configKey];
    const labelOnPRUpdate =
      "labelOnPRUpdates" in labelerFlags ? labelerFlags.labelOnPRUpdates : true;
    ...
    if (!(labelOnPRUpdate || context.payload.action === "opened")) {
      context.log.info(
        "Labeling on PR Update is disabled, skipping Labeler ..."
      );
      return;
    }
