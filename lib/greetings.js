...
async function commentOnfirstPR(context, config) {
  ...
  if (configKey in config) {
    ...
    await context.github.paginate(
      ...
      (res) =>
        res.data.items.forEach((pr) => {
          allPRs.add(pr);
        })
    );

    // Filter current PR
    const allPreviousPRs = Array.from(allPRs).filter(
      (pr) => pr.number !== context.payload.pull_request.number
    );

    context.log.info(
      `🎛 Number of PRs excluding the current: ${allPreviousPRs.length}`
    );
    ...
    if (allPreviousPRs.length === 0) {
      try {
        if (allPreviousPRs) {
          context.log.info(
            `📝 Adding comment on ${context.payload.pull_request.html_url}: ${firstPRWelcomeComment}`
          );
          context.github.issues.createComment(
            context.issue({ body: firstPRWelcomeComment })
          );
        }
      } catch (err) {
        if (err.code !== 404) {
...
async function commentOnfirstPRMerge(context, config) {
  ...
  if (context.payload.pull_request.merged) {
    if (configKey in config) {
      ...
      // Filter current PR
      const mergedPRs = Array.from(allMergedPRs).filter(
        (pr) => pr.number !== context.payload.pull_request.number
      );

      context.log.info(
        `🎛 Number of Merged PRs excluding the current: ${mergedPRs.length}`
      );
      ...
      if (mergedPRs.length === 0) {
        try {
          if (firstPRMergeComment) {
            context.log.info(
              ` 📝 Adding comment on ${context.payload.pull_request.html_url}: ${firstPRMergeComment}`
            );
            context.github.issues.createComment(
              context.issue({ body: firstPRMergeComment })
            );
          }
        } catch (err) {
          if (err.code !== 404) {
...
async function commentOnfirstIssue(context, config) {
  ...
  if (configKey in config) {
    ...
    context.log.info(
      `🎛 Number of Issues excluding the current: ${allPreviousIssues.length}`
    );
    ...
    if (allPreviousIssues.length === 0) {
      try {
        if (allPreviousIssues) {
          context.log.info(
            ` 📝 Adding comment on ${context.payload.issue.html_url}: ${firstIssueWelcomeComment}`
          );
          context.github.issues.createComment(
            context.issue({ body: firstIssueWelcomeComment })
          );
        }
      } catch (err) {
        if (err.code !== 404) {
