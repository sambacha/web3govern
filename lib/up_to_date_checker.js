...
async function checkUpToDate(context, config) {
  ...
  if (configKey in config) {
    ...
    if (modifiedFiles.find((file) => matcher.ignores(file))) {
      context.log.info(
        " ✅ Found files matching conditions. Checking if need rebase."
      );

      // Get parent commit of the first commit in the PR
      const parentCommitId = await _getParentOfFirstCommitOfPR(context, issue);

      const branchesForHeadCommit = await _listBranchesForHeadCommit(
        context,
        parentCommitId
      );
      ...
      if (branchesForHeadCommit.includes(targetBranch)) {
        ...
        await _createStatus(
          ...
          checkerName,
          "success",
          `PR is up to date with ${targetBranch}`
        );
      } else {
        context.log.info(
          `🆘 PR branch is not up to date with ${targetBranch}. Need rebase`
        );
        await _createStatus(
          context,
          pr,
          checkerName,
          "failure",
          `PR is not up to date with ${targetBranch}. Please rebase.`
        );
      }
    }
  }
...
async function _getParentOfFirstCommitOfPR(context, issue) {
  ...
  const getFirstCommit = await context.github.repos.getCommit(
    context.repo({ commit_sha: firstCommitId })
  );
...
async function _listBranchesForHeadCommit(context, commit) {
  const getBranches = await context.github.repos.listBranchesForHeadCommit(
    context.repo({ commit_sha: commit })
  );
