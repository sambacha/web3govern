...
async function insertIssueLinkInPrDescription(context, config) {
  ...
  if (configKey in config) {
    ...
    const placeholderRegexp = new RegExp(
      insertIssueLinkInPrDescription.descriptionIssuePlaceholderRegexp,
      "m"
    );
    ...
    if (matchedPlaceholderArray == null) {
      context.log.warn(
        " ⛔️ Placeholder not found in the body. Body:\n",
        pr.body,
        "Regexp:",
        placeholderRegexp
      );
    ...
    if (matchedPlaceholderArray.length < 2) {
      context.log.warn(
        " ⛔️ The descriptionIssuePlaceholderRegexp regexp should " +
          "contain a match group (). Regexp:",
        placeholderRegexp
      );
      return;
    }
    ...
    const linkPlaceholderPosition = matchedPlaceholderArray[0].indexOf(
      matchedPlaceholderArray[1]
    );
    ...
    for (const matcher in insertIssueLinkInPrDescription.matchers) {
      ...
      if (matchedIssueIdArray == null) {
        context.log.debug(
          "🆘 Issue ID not found in the title. Title:",
          pr.title,
          "Regexp:",
          issueIdRegexp
        );
      ...
      if (matchedIssueIdArray.length < 2) {
        context.log.warn(
          "⛔️ The titleIssueIdRegexp regexp should contain a match group: (): Regexp:",
          issueIdRegexp
        );
        continue;
      }
      ...
      const newBody =
        pr.body.substr(0, placeholderPosition + linkPlaceholderPosition) +
        linkString +
        pr.body.substr(
          placeholderPosition + linkPlaceholderPosition + linkPlaceholderLength
        );
    ...
    context.log.debug(
      "🔃 Skip insertIssueLinkInPrDescription - not configured!"
    );
  }
}
