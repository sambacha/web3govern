...
async function insertIssueLinkInPrDescription (context, config) {
  ...
  if (configKey in config) {
    ...
    if (matchedPlaceholderArray == null) {
      context.log.warn(' ⛔️ Placeholder not found in the body. Body:\n', pr.body,
        'Regexp:', placeholderRegexp)
      return
    }

    if (matchedPlaceholderArray.length < 2) {
      context.log.warn(' ⛔️ The descriptionIssuePlaceholderRegexp regexp should ' +
    ...
    for (const matcher in insertIssueLinkInPrDescription.matchers) {
      ...
      if (matchedIssueIdArray.length < 2) {
        context.log.warn('⛔️ The titleIssueIdRegexp regexp should contain a match group: (): Regexp:',
          issueIdRegexp)
        continue
      }
